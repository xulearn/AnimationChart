...
export async function PlayBarChart() {
  try {
    await Excel.run(async context => {
      ...
      toolTable.onChanged.add(toolTableChanged);
      
      targetTable = toolTable;
      let targetRange = toolTable.columns.getItemAt(1).getDataBodyRange();
      targetRange.load("address");
      await context.sync();
      let targetStr = targetRange.address.split("!"); //sheet!aa:bb
      targetAddr = targetStr[1];
      ...
      for(let i=2;i<totalColumnCount;++i){
        ...
        for(let step = 1; step<= splitIncreasement; step++){    //2
...
async function toolTableChanged(event: Excel.TableChangedEventArgs) {
  
    let changedAddr = event.address;  //aa:bb
   
    if(changedAddr == targetAddr){
      // await sortTable(targetTable);
      targetTable.sort.apply([{ key: 1, ascending: true }], true);
      // flag |= 1;
      // flag = await reverseFlag(flag);
    }
  
}

// async function sortTable(table:Excel.Table){
//   table.sort.apply([{ key: 1, ascending: true }], true);
// }

// async function reverseFlag(falg:number){
//   return falg|1;
// }
